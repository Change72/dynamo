...
def build_sampling_params(
    ...
    # remote_kv_reuse_plan from request.extra_args and pass it through as
    ...
    try:
        ...
        if isinstance(_router_plan, dict):
            ...
            _logging.getLogger(__name__).info(
                "native router plan adapted: req=%s source_control_endpoint=%s blocks=%s",
                request.get("request_id", "?"),
                _router_plan.get("source_control_endpoint"),
                len(_router_plan.get("block_hashes") or []),
            )
